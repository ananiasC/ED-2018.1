...
void furar(matchar &mat, Par par){
    ...
    while(pilha.size() != 0){
        ...
        for(Par vizinho : shuffle(getNeib(topo))){
            if(countOpen(mat, vizinho) == 1 && mat.equals(vizinho,PAREDE))
                furaveis.push_back(vizinho);
        }
        if(furaveis.size() == 0)
            pilha.pop_back();
        else{
            Par escolhido = furaveis[rand() % furaveis.size()];
